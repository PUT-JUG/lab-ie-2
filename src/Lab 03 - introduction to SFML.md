...
Next, follow the instructions for adding SFML to your project in Visual Studio, which can be found at https://www.sfml-dev.org/tutorials/3.0/getting-started/visual-studio/#installing-sfml.



## Adding SFML (Linux)

Download SFML from the official repository. First, remove any existing SFML folder and then download the library:

```bash
rm -rf ~/SFML-3.0.2-Linux.GCC
cd ~
wget https://github.com/SFML/SFML/releases/download/3.0.2/SFML-3.0.2-Linux.GCC.tar.gz
tar -xzf SFML-3.0.2-Linux.GCC.tar.gz
mv SFML-3.0.2-Linux.GCC* SFML-3.0.2
```

After extraction, the library will be located in `~/SFML-3.0.2`. Modify `task.json` to include the path to the SFML installation, like this:
    ...
    "tasks": [
        {
            ...
            "args": [
                ...
                "-L",
                "~/SFML-3.0.2/lib",
                "-Wl,-rpath,/home/put/SFML-3.0.2/lib",
                "-lsfml-graphics",
                "-lsfml-window",
                "-lsfml-system"
            ],
