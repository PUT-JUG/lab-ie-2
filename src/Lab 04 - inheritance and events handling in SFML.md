...
{
    // This is the event for closing the window, it is generated when user clicks the close button on the window
    if (event->is<sf::Event::Closed>()) 
        window.close();

    // This is the event for pressing a key on the keyboard
    ...
    // This is the event for pressing a mouse button
    else if (const auto* mouseButtonPressed = event->getIf<sf::Event::MouseButtonPressed>())
    {
        if (mouseButtonPressed->button == sf::Mouse::Button::Middle)
        {
            sf::cout << "Middle mouse button pressed" << std::endl;
        }
}
```
